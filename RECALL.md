...
| `ops` | **운영 화면(dnalabs.kr/admin)이 쓰는 유일한 창구.** 회사 셋을 한 화면에서 보려면 RLS 를 넘어야 하는데, 정책마다 예외를 뚫는 대신 힘을 이 파일 하나에 가뒀습니다. `operators` 에 없으면 403 — 왜 막혔는지 알려 주지 않습니다 |
...
`Re:Store` 는 인원이 아니라 **점포 수**가 값을 정해 `storeTiers` 를 따로 둡니다
(10 / 30 / 50개, 사다리는 `tiers` 와 같은 것). 둘 다 해당하면 **더 위의 줄**을 씁니다.

⚠ `services[].note` 는 **화면에 그대로 나갑니다.** 표 이름 같은 우리끼리 쓰는 말을 넣으면
손님이 봅니다 — `add-service` 쪽 카드에 "storeTiers 에서 고릅니다" 가 찍혀 나온 적이 있습니다.

### 서비스 바꾸기 단추 (네 앱)

위쪽 막대 오른쪽. 산 것은 바로 넘어가고, 안 산 것은 `dnalabs.kr/add-service?want=…&from=…` 로 보냅니다.
**네 앱이 같은 코드**입니다 — 하나를 고치면 넷을 다 고칩니다(로그인 토글과 같은 규칙).
흐리게 그리는 것은 안내일 뿐 자물쇠가 아닙니다. 회사 코드는 주소에 싣지 않습니다.
...
| ~~통신판매업 신고~~ | **2026-09-08 에 났습니다** — 제 `2026-경기파주-3695` 호 (파주시). 네 쪽 꼬리말에 들어가 있습니다. 신고증은 `~/Documents/통신판매업 신고증(디엔에이랩스).pdf` |
...
**2026-09-14 부터 두 칸입니다.** 왼쪽은 로고 · 토글 · 로그인 칸,
오른쪽(`#gside`)은 **고른 서비스의 소개와 실제 화면**입니다.
토글을 누르면 `setPick()` 이 제목 · 단추 글자 · 오른쪽 판 · 색을 함께 바꿉니다.

| 무엇 | 어디 |
|---|---|
| 판 넷 (`.sv[data-app]`) | 앱 파일의 `#gside` 안. 넷 다 같은 글 |
| 색 | `const ACCENT` — Re:Bind 파랑 · Re:Call 초록 · Re:Store 주황 · Re:O-S 보라 |
| 그림 | `https://dnalabs.kr/shot-*.png` — `network-dna/build-shots.sh` 가 찍은 실제 앱 화면 |

⚠ 그림은 `data-src` 로 두고 **화면이 넓을 때만** 받습니다(`innerWidth > 880`).
폰에서는 판을 로그인 아래로 내리고 그림을 감추므로 받을 이유가 없습니다.

⚠ 새 CSS 는 `#gate .box.wide …` 로 씁니다. 위에 있는 옛 규칙(`#gate .box …`)을
확실히 덮으려는 것입니다 — 미디어쿼리 차례에 기대면 화면 폭에 따라 옛 배치가 나옵니다.
...
## 8.5 운영 화면 (dnalabs.kr/admin)

**우리만 씁니다.** 고객사 관리자용 설정은 각 앱 안에 따로 있습니다.

| | |
|---|---|
| 화면 | `web/admin.html` — 파일 하나. 로그인 보관 자리는 `dnalabs-ops` (앱은 `ndna-auth`) |
| 창구 | `supabase/functions/ops` — 여기 없는 일은 아무도 못 합니다 |
| 누가 | `operators` 표에 있는 사람. `bash add-operator.sh <회사코드> <아이디>` |

### ⚠ 고객사 관리자를 운영자로 삼지 마세요

`BKT`·`ACTIVA`·`9DORO` 의 `admin` 은 **고객사 것**입니다. 그 계정을 운영자로
두면 고객사가 우리 운영 화면에 들어와 남의 회사 목록을 보게 됩니다.
우리 계정은 `DNALABS` 회사 아래에 따로 둡니다.

### `operators` 에 정책이 없는 것은 의도입니다

RLS 를 켜 두고 정책이 없으면 브라우저에서는 아무것도 못 합니다 —
로그인한 사람이 "나는 운영자인가" 조차 직접 물어볼 수 없습니다.
물어보려면 `ops` 를 거쳐야 하고, 그러면 기록이 남습니다.
**정책을 더하지 마세요.**

### 할 수 있는 일

회사 만들기(`apps` 까지 한 번에) · 서비스 켜고 끄기 · 회사 멈추기 ·
계정 추가/재발급/중지 · 사용량 보기 · 기록 100건.

구독은 **있는 그대로만** 보여 줍니다(7장). 요금제는 Re:Call 것만 있고
PG 계약 전이라 담을 내용이 없습니다.
